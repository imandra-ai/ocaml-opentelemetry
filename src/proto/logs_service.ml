...
let rec pp_export_logs_partial_success fmt (v:export_logs_partial_success) = 
  let pp_i fmt () =
    ...
    if not (export_logs_partial_success_has_rejected_log_records v) then Format.pp_print_string fmt "(* absent *)";
    Pbrt.Pp.pp_record_field ~first:false "error_message" Pbrt.Pp.pp_string fmt v.error_message;
    if not (export_logs_partial_success_has_error_message v) then Format.pp_print_string fmt "(* absent *)";
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
